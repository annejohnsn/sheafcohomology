...
Note that the rows are still split exact (because each $I^n_F$ is injective and since $\Gamma I_F^n \oplus I_G^n = I_F^n \oplus I_G^n (X) = I_F^n (X) \oplus I_G^n $ ), and the restriction of the maps in the rows to the homology groups is again split exact:
...
Then $p^*$ gives an isomorphism from $H^p(T; \Delta_T\mathbb{Z})$ to $H^p(X; p^*\Delta_T\mathbb{Z})$.*** just see what $p^*\Delta_T\mathbb{Z})$ is an compute the resolution***% Note that  since $X = [0,1] \times [0,1]$ is contractible, we have that $id_X$ is homotopic  to $f: X \rightarrow \{*\}$ and so $H^n({*};p^*\Delta_T\mathbb{Z}) \xrightarrow{\sim} Hn(X,f^*p^*\Delta_T\mathbb{Z})$. This gives: 
\[
